...
void publish_message(MQTT::Client<MQTTNetwork, Countdown>* client) {
    ...
    if(mode==1){
      ...
      sprintf(buff, "#%2d Mode: 1, gesture_number: %d", message_num, confirmed_gesture_number);
    }else if(mode==2){
      if(first_tilt_send==0){
        sprintf(buff, "#%2d Mode: 2, %5d,%5d,%5d, %2.2f", message_num, now_a_XYZ[0],now_a_XYZ[1],now_a_XYZ[2],now_tilt_degree);
      }else{
        sprintf(buff, "#%2d Mode: 3", message_num);
      }
...
void tilt_thread_any_point(){
    ...
    const char* host = "192.168.160.29";//172.20.10.2
    ...
    //for(i=0;stop_sample==0;i=i+1)
        ...
        now_tilt_degree=(acos((double)now_a_XYZ[2]/1050))/3.14159*180;
        printf("%d, %d, %d, %2.2f\n", now_a_XYZ[0], now_a_XYZ[1], now_a_XYZ[2], now_tilt_degree);
        /*uLCD.cls();
        uLCD.locate(1,1);
        uLCD.printf("x: %d",now_a_XYZ[i][0]);
        uLCD.locate(1,3);
        uLCD.printf("y: %d",now_a_XYZ[i][1]);
        uLCD.locate(1,5);
        uLCD.printf("z: %d",now_a_XYZ[i][2]);
        uLCD.locate(1,7);
        uLCD.printf("Angle: %f",now_tilt_degree[i]);*/
